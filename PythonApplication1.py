...
def send_monitoring_email(updates, sender_email, recipients):
    """Send monitoring updates email"""
    if not updates:
    ...
    for update in updates:
        html_rows += f"""
        <tr>
            <td style="padding:6px 10px;border-bottom:1px solid #eee;">{update['Ticker']}</td>
            <td style="padding:6px 10px;border-bottom:1px solid #eee;">{update['Action']}</td>
            <td style="padding:6px 10px;border-bottom:1px solid #eee;">{update['Price']:.2f}</td>
            <td style="padding:6px 10px;border-bottom:1px solid #eee;">{update.get('PnL', '')}</td>
            <td style="padding:6px 10px;border-bottom:1px solid #eee;">{update.get('Reason', '')}</td>
        </tr>
        """
    ...
    <html>
    <body style="font-family:Segoe UI, Arial, sans-serif;">
      <h2 style="margin-bottom:6px;">Portfolio Monitoring Update</h2>
      <div style="color:#555;margin-bottom:12px;">Generated: {date_str}</div>
      <table cellpadding="0" cellspacing="0" style="border-collapse:collapse;min-width:520px;">
        <thead>
          <tr style="background:#f5f5f5;">
            ...
            <th style="text-align:left;padding:8px 10px;">Price</th>
            <th style="text-align:left;padding:8px 10px;">P&L</th>
            <th style="text-align:left;padding:8px 10px;">Reason</th>
          </tr>
        </thead>
        <tbody>
          {html_rows}
    ...
    # Use a modified version of send_top_buys_email for monitoring
    return send_top_buys_email(
        [],  # Empty top_buys since we're using custom HTML
        sender_email,
        recipients,
        subject=f"Portfolio Update - {now_ist.strftime('%Y-%m-%d %H:%M')}",
        sender_name="Quant Signals Bot"
    )
...
def run_discovery_mode():
    ...
    if top_buys:
        ...
        if sender_email and recipients:
            ...
            print("Email not sent: Missing configuration")
...
def run_monitoring_mode():
    ...
    if updates:
        ...
        if sender_email and recipients:
            send_monitoring_email(updates, sender_email, recipients)
